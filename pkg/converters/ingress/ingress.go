...
func NewIngressConverter(options *ingtypes.ConverterOptions, haproxy haproxy.Config, globalConfig map[string]string) Config {
	defaultSvc := strings.Split(options.DefaultBackend, ":")
	defSvcName := defaultSvc[0]
	defSvcPort, _ := strconv.Atoi(defaultSvc[1])
	c := &converter{
		haproxy:             haproxy,
		options:             options,
	...
	backend, _ := c.addBackend(defSvcName, defSvcPort, &ingtypes.BackendAnnotations{})
